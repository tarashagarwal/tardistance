...
VALUE distance(VALUE self,VALUE str,VALUE rarray) {
...
VALUE bulkDistance(VALUE self,VALUE rstr,VALUE rarray) {
	//int arrayLen = RARRAY_LEN(rarray);
	char **word_list;
	word_list = (char**)getWordsArray(rarray);
...
void Init_skynet()
{
  ...
  rb_define_global_function("distance",distance,2);
